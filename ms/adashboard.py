...
class DashboardApp(tk.Tk):
    def __init__(self):
        ...
        self.frame.pack(padx=20, pady=120)
    ...
    def create_widgets(self):
        ...
        button_height = 3
        self.add_button = tk.Button(self.frame, text="Add", font=("Georgia", 10), command=self.add_meeting, height=button_height, bg='light blue')
        self.modify_button = tk.Button(self.frame, text="Modify", font=("Georgia", 10), command=self.modify_meeting, height=button_height, bg='light blue')
        self.delete_button = tk.Button(self.frame, text="Delete", font=("Georgia", 10), command=self.delete_meeting, height=button_height, bg='light blue')
        ...
        self.add_button.grid(row=3, column=0, padx=20, pady=20, sticky="n")
        self.modify_button.grid(row=3, column=1, padx=20, pady=20, sticky="n")
        self.delete_button.grid(row=3, column=3, padx=20, pady=20, sticky="n")
        self.search_button.grid(row=2, column=0, columnspan=1, padx=20, pady=20, sticky="n")
        self.display_button.grid(row=2, column=2, columnspan=2, padx=20, pady=20, sticky="n")

    def add_meeting(self):
        self.destroy()
        subprocess.run(["python", "add.py"], shell=True)

    def modify_meeting(self):
        self.destroy()
    ...
    def search_meeting(self):
        self.destroy()
        subprocess.run(["python", "asearch.py"], shell=True)

    def display_meetings(self):
        self.destroy()
        subprocess.run(["python", "adisplay.py"], shell=True)

...
class AdminApp:
    def __init__(self, root):
        ...
        self.frame.pack(padx=20, pady=150,anchor='c')
        
        style = ttk.Style(self.root)
        style.theme_use('clam')

        self.root.p1 = tk.PhotoImage(file='admin.png')
    ...
    def sign_up(self):
        self.root.destroy()
        subprocess.run(["python", "asign_up.py"], shell=True)

    def sign_in(self):
        self.root.destroy()
        subprocess.run(["python", "asign_in.py"], shell=True)

    def create_ui(self):
        label = tk.Label(self.frame, text="Admin", font=("Georgia", 18, "bold"), bg='light blue')
